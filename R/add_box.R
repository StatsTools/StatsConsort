...
#' @param id A character with de the id of the box. Must be unique.
...
#'  add_box(id = 'box1', x = c(10, 5), y = c(50, 3), align = 'c', text = 'Box 1')
add_box <- function(consort, id, x, y, align, text, fontsize = 8) {
 ...
 stopifnot("`id` must be unique." = !(id %in% sapply(consort@boxes, function(box) box@id)))
 # -----------------------------------------------------------------------------

 return(add_box_method(x = consort, id = id, x_center = x[1], x_hl = x[2], y_center = y[1], y_hl = y[2], align = align, text = text, fontsize = fontsize))
}
